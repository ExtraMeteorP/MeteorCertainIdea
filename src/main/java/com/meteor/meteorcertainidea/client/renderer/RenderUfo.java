...
public class RenderUfo extends EntityRenderer<EntityUfo> {
    ...
    private final ModelUfo model;

    public RenderUfo(EntityRendererProvider.Context ctx) {
        super(ctx);
        model = new ModelUfo(ctx.bakeLayer(ModModelLayers.UFO));
    }

    @Override
    public void render(EntityUfo e, float yaw, float partialTicks, PoseStack pos, MultiBufferSource buffers, int light) {
        pos.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(EntityUfo p_114482_) {
        return new ResourceLocation(LibMisc.MOD_ID, "textures/model/ufo.png");
    }
